    ...
    <script>
      ...
      function searchFiles() {
			...
			for (i = 0; i < li.length; i++) {
				...
				if (txtValue.toUpperCase().indexOf(filter) > -1 && (selectedCategory === "" || a.querySelector('.file-category').textContent.includes(selectedCategory))) {
					li[i].style.display = "";
				} else {
            ...
            .then(data => {
                if (data.status === 'success') {
                    Swal.fire({
                        ...
                        addFileToContainer(data.fileName, data.filePath, data.fileId, category); // Pass category to update UI
                    });
                } else {
                    Swal.fire({
                        icon: 'error',
