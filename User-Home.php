...
function getUploadedFiles($user_id, $conn) {
    $uploadedFiles = array();
    $stmt = $conn->prepare("SELECT user_id, file_name, file_path FROM user_files WHERE user_id = ?");
        ...
        if (!file_exists($uploadPath)) {
            if (move_uploaded_file($_FILES['pdfFile']['tmp_name'], $uploadPath)) {
                ...
                if ($stmt->execute()) {
                    ...
                    $stmt->close();
                    echo json_encode(array('status' => 'success', 'fileName' => $fileName, 'filePath' => $uploadPath, 'file_id' => $file_id));
                } else {
...
<body>
    <div class="side-panel">
        ...
        <ul>
			...
			<li><a href="user-login.php" onclick="logout()"><i class="fas fa-sign-out-alt"></i> Sign Out</a></li>
    ...
    <div class="upload-form">
        <button class="upload-button" onclick="showUploadModal()">Upload File</button>
    </div>
    ...
    <div id="uploadModal" style="display: none;">
        ...
        <select id="categorySelect">
            <option value="uncategorized">IT</option>
            <option value="documents">Science</option>
            <option value="images">History</option>
    ...
    <div class="main-content">
		...
		<div class="content-item" id="file_<?php echo $file['user_id']; ?>">
			<div class="content-details">

				<a href="view.php?user_id=<?php echo $file['user_id']; ?>"><?php echo $file['file_name']; ?></a>

				<button class="delete-button" onclick="deleteFile('<?php echo $file['user_id']; ?>')"><i class="fas fa-trash"></i></button>
			</div>
		</div>
	<?php endforeach; ?>
    ...
    if (confirm("Are you sure you want to delete this file?")) {
        ...
        .then(data => {
            if (data.status === 'success') {
                var fileElement = document.getElementById("file_" + user_id);
                if (fileElement) {
                    fileElement.remove();
                    Swal.fire({
                        icon: 'success',
...
function openFileRecently(fileName, userId) {
    ...
    displayRecentlyOpenedFiles(userId);
}

function displayRecentlyOpenedFiles(userId) {
    var recentlyOpenedKey = 'recentlyOpenedFiles_' + userId;
    ...
    recentlyOpenedFiles.forEach(fileName => {
        ...
        link.href = "uploads/user_" + userId + "/" + fileName;
        link.target = "_blank";
